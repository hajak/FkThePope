...
describe('resolveTrick', () => {
  ...
  it('should handle all cards being the same suit as trump', () => {
    const trick: PlayedCard[] = [
      { card: { suit: 'spades', rank: '5' }, playedBy: 'north', faceDown: false, playedAt: 0 },
      { card: { suit: 'spades', rank: 'Q' }, playedBy: 'east', faceDown: false, playedAt: 1 },
      { card: { suit: 'spades', rank: '3' }, playedBy: 'south', faceDown: false, playedAt: 2 },
      { card: { suit: 'spades', rank: 'K' }, playedBy: 'west', faceDown: false, playedAt: 3 },
    ];

    expect(resolveTrick(trick, 'spades')).toBe('west');
  });

  it('should handle 10 vs face cards correctly (J > 10)', () => {
    const trick: PlayedCard[] = [
      { card: { suit: 'hearts', rank: '10' }, playedBy: 'north', faceDown: false, playedAt: 0 },
      { card: { suit: 'hearts', rank: 'J' }, playedBy: 'east', faceDown: false, playedAt: 1 },
      { card: { suit: 'hearts', rank: '9' }, playedBy: 'south', faceDown: false, playedAt: 2 },
      { card: { suit: 'hearts', rank: '8' }, playedBy: 'west', faceDown: false, playedAt: 3 },
    ];

    expect(resolveTrick(trick, 'clubs')).toBe('east');
  });

  it('should handle lowest trump beating highest lead suit', () => {
    const trick: PlayedCard[] = [
      { card: { suit: 'hearts', rank: 'A' }, playedBy: 'north', faceDown: false, playedAt: 0 },
      { card: { suit: 'hearts', rank: 'K' }, playedBy: 'east', faceDown: false, playedAt: 1 },
      { card: { suit: 'clubs', rank: '2' }, playedBy: 'south', faceDown: false, playedAt: 2 }, // Lowest trump
      { card: { suit: 'hearts', rank: 'Q' }, playedBy: 'west', faceDown: false, playedAt: 3 },
    ];
  ...
  it('should handle all players discarding face-down (leader wins)', () => {
    const trick: PlayedCard[] = [
      { card: { suit: 'hearts', rank: '5' }, playedBy: 'north', faceDown: true, playedAt: 0 },
      { card: { suit: 'hearts', rank: 'A' }, playedBy: 'east', faceDown: true, playedAt: 1 },
      { card: { suit: 'hearts', rank: 'K' }, playedBy: 'south', faceDown: true, playedAt: 2 },
      { card: { suit: 'hearts', rank: 'Q' }, playedBy: 'west', faceDown: true, playedAt: 3 },
    ];

    expect(resolveTrick(trick, 'spades')).toBe('north');
  });

  it('should handle mixed face-down with single face-up winner', () => {
    const trick: PlayedCard[] = [
      { card: { suit: 'hearts', rank: 'A' }, playedBy: 'north', faceDown: true, playedAt: 0 },
      { card: { suit: 'hearts', rank: '3' }, playedBy: 'east', faceDown: false, playedAt: 1 },
      { card: { suit: 'hearts', rank: 'K' }, playedBy: 'south', faceDown: true, playedAt: 2 },
      { card: { suit: 'hearts', rank: 'Q' }, playedBy: 'west', faceDown: true, playedAt: 3 },
    ];

    expect(resolveTrick(trick, 'spades')).toBe('east'); // Only face-up card wins
  });
});
